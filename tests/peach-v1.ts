...
import {
  ...
  KAMINO_LENDING,
  KAMINO_RESERVE_FARM_STATE_USDC,
  KAMINO_RESERVE_USDC,
...
describe("peach-v1", () => {
  ...
  describe("Native happy tests", () => {
    ...
    let deposit_amount1 = new anchor.BN(100),
      deposit_amount2 = new anchor.BN(50);
    let withdraw_amount1 = new anchor.BN(10),
      withdraw_amount2 = new anchor.BN(30);
    ...
    before(async () => {
      ...
      console.log(
        ...
        await connection.getBalance(programWallet.publicKey)
      );
    ...
    describe("Create accounts and register token", () => {
      ...
      it("Creates a peach account", async () => {
        try {
          ...
          const account = await program.account.peachAccountFixed.fetch(
            peachAccount
          );
    ...
    describe.skip("Native Token operations", () => {
      it("Deposits a token: user1", async () => {
        try {
          ...
          const userAccount = await program.account.peachAccountFixed.fetch(
            peachAccount
          );
      ...
      it("Withdraws a token: user1", async () => {
        try {
          ...
          const vaultBalance = await connection.getTokenAccountBalance(vault);
          const peachAccount1 = await program.account.peachAccountFixed.fetch(
            peachAccount
          );
          // assert.equal(peachAccount1.netDeposits.toNumber(), deposit_amount1.sub(withdraw_amount));
          assert.equal(
            vaultBalance.value.amount,
            deposit_amount1.sub(withdraw_amount1).toString()
          );
        } catch (err) {
      ...
      it.skip("Deposits a token: user2", async () => {
        try {
          await tokenDepositIntoExisting(
            deposit_amount2,
            market,
            peachAccount2,
            bank,
      ...
      it.skip("Withdraws a token: user2", async () => {
        try {
          ...
          const vaultBalance = await connection.getTokenAccountBalance(vault);
          const peachAccount = await program.account.peachAccountFixed.fetch(
            peachAccount2
          );
    ...
    describe("Kamino operations", () => {
      ...
      before(async () => {
        ...
        console.log(
          "User Kamino USDC Token Account: ",
          user_kamino_usdc_token_account.toBase58()
        );
        console.log(
          "Reserve Liquidity Supply PDA: ",
          reserveLiquiditySupplyPda.toBase58()
        );
        console.log(
          "Reserve Deposit Collateral PDA: ",
          reserveDepositCollateralPda.toBase58()
        );
        console.log(
          "Lending Market Authority PDA: ",
          lendingMarketAuthorityPDA.toBase58()
        );
        console.log("Obligation Farm PDA: ", obligationFarm.toBase58());
      });

      it("Init User MetaData", async () => {
        const lookupTableAddress = await createLookupTableAddress(
          program.provider.connection,
          user
        );

        const signature = await program.methods
          .kaminoInitUserMetadata(lookupTableAddress)
          .accounts({
        ...
        const data = await kaminoProgram.account.UserMetadata.fetch(
          userMetaDataPDA
        );
        assert.equal(
          lookupTableAddress.toBase58(),
          data.userLookupTable.toBase58()
        );
      ...
      it("Init Obligation", async () => {
        ...
        const obligationAccount = await kaminoProgram.account.Obligation.fetch(
          obligationPDA
        );
        assert.equal(
          obligationAccount.owner.toBase58(),
          user.publicKey.toBase58()
        );
        assert.equal(
          obligationAccount.lendingMarket.toBase58(),
          lendingMarket.toBase58()
        );
        assert.equal(obligationAccount.tag.toNumber(), args.tag);
        delayForMainnet();
      });

      it("Init User Obligation Farm for Reserve", async () => {
        const signature = await program.methods
          .kaminoInitObligationFarmForReserve(mode)
          .accounts({
            payer: user.publicKey,
            market: market,
            peachAccount: peachAccount,
      ...
      it("Deposit to kamino", async () => {
        ...
        const ixRefreshReserve = await kaminoProgram.methods
          .refreshReserve()
          .accounts({
            reserve: KAMINO_RESERVE_USDC,
            lendingMarket: KAMINO_LENDING_MAIN_MARKET,
        ...
        const txRefreshObligation = await kaminoProgram.methods
          .refreshObligation()
          .accounts({
            lendingMarket: KAMINO_LENDING_MAIN_MARKET,
            obligation: obligationPDA,
        ...
        const IxRefreshObligationFarmsForReserve = await kaminoProgram.methods
        ...
        const depositInstruction = await program.methods
          .kaminoDeposit(deposit_amount, 0)
          .accounts({
            market: market,
      ...
      it("Withdraw from kamino", async () => {
        ...
        const withdrawInstruction = await program.methods
          .kaminoWithdraw(withdraw_amount, true)
          .accounts({
            market: market,
            signer: user.publicKey,
            obligation: obligationPDA,
    ...
    describe.skip("Deregister and close accounts", () => {
      ...
      it("Close peach account", async () => {
        try {
          ...
          assert.ok(
            await program.account.peachAccountFixed
              .fetch(peachAccount)
              .then(() => false)
              .catch(() => true)
          );
      ...
      it("Deregister a token", async () => {
        try {
          ...
          const mintInfoAccount = await program.account.mintInfo
            .fetch(mintInfo)
            .then(() => false)
            .catch(() => true);
          const bankAccount = await program.account.bank
            .fetch(bank)
            .then(() => false)
            .catch(() => true);
