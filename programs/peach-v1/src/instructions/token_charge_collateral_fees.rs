...
pub fn token_charge_collateral_fees(ctx: Context<TokenChargeCollateralFees>) -> Result<()> {
    token_charge_collateral_fees_internal(
        ...
        ctx.accounts.market.load()?.deref(),
    ...
    Dynamic: DerefOrBorrowMut<[u8]> + DerefOrBorrow<[u8]>,
{
    ...
    if market.collateral_fee_interval == 0 {
    ...
    if now_ts < last_charge_ts + market.collateral_fee_interval {
        return Ok(());
    }
    account.fixed.last_collateral_fee_charge = now_ts;
    ...
    let charge_seconds = (now_ts - last_charge_ts).min(2 * market.collateral_fee_interval);
    ...
    // the token positions.
    let token_balances = health_cache.effective_token_balances(HealthType::Maint);
