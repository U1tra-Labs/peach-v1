...
pub struct TokenUpdateIndexAndRate<'info> {
    #[account(
        constraint = market.load()?.is_ix_enabled(IxGate::TokenUpdateIndexAndRate) @ PeachError::IxIsDisabled,
    )]
    pub market: AccountLoader<'info, Market>, // Required for market metadata parsing
